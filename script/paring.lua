...
local function test(event)
	...
	tunnel = player.surface.find_entity('TrainTunnelT1-entrance', event.cursor_position) --need capsule to detect tunnel constantly, and the capsule id should be stored as key of global.Tunnels
	...
	if tunnel ~= nil then
		if global.Tunnels[tunnel.unit_number].paired == false and global.Tunnels[tunnel.unit_number].pairing == false and not user_under_paring(player.index) then
			global.Paring[tunnel.unit_number] = {}
			global.Tunnels[tunnel.unit_number].pairing = true
			global.Paring[tunnel.unit_number].timer = 0
			global.Paring[tunnel.unit_number].player = player.index
